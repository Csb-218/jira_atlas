    ...
    <>
      ...
      <div className="relative inline-flex">
        ...
        <ul
          className={`${
            isOpen ? "flex" : "hidden"
          } absolute top-full z-10 right-1 mt-1 w-72 list-none flex-col rounded bg-white py-2 shadow-md shadow-slate-500/10 `}
        >
          {navigationItems.map((item, index) => {
            if (item?.separator) {
              return (
                <li role="separator" className="border-b border-slate-200"></li>
              )
            } else
              return (
                <li key={index}>
